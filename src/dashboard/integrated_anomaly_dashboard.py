...
def process_video_and_save_to_mongodb(video_path, progress_callback=None):
    ...
    try:
        # Try loading pose model with safe globals first
        try:
        ...
        # Try loading object detection model with safe globals first
        try:
        ...
        try:
            ...
            try:
                ...
                document_model.set_classes(document_classes)
                st.success("YOLO models loaded successfully with fallback method")
        except Exception as e:
            st.warning(f"Could not load YOLO-World model for document detection: {e}")
            st.warning("Document detection will be disabled")
    except Exception as e:
